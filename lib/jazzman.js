...
/**
 * jazzman Spies are test doubles that can act as stubs, spies, fakes or when used in an expecation, mocks.
 *
 * Spies should be created in test setup, before expectations.  They can then be checked, using the standard jazzman
 ...
 *
 * Note: Do <b>not</b> call new jazzman.Spy() directly - a spy must be created using spyOn, jazzman.createSpy or jazzman.createSpyObj.
 *
 * @example
 * // a stub
...
jazzman.getGlobal().waitForSpyCalled = waitsForSpyCalled;
...
jazzman.setOptions = function(jsonString) {
    jazzman._options = JSON.parse(jsonString);
};

jazzman.getOptions = function() {
    return jazzman._options || {};
};

jazzman.initDebug = function() {
    var spec = jazzman.getOptions().spec;
...
jazzman.generateDebuggableBlock = function(fn) {
    return function() {
        debugger;
...
jazzman.showDebugPrompt = function(callback) {
...
jazzman.util.getFileFromContextMapping = function(file) {
    ...
    if (file && contextMapping) {
        var origin = jazzman.util.getOrigin();
...
jazzman.Env.prototype._compareObjects = function(a, b, mismatchKeys, mismatchValues) {
    ...
    for (property in b) {
        ...
        if (!this._equals(a[property], b[property], mismatchKeys, mismatchValues)) {
            mismatchValues.push("'" + property + "' was '" + (b[property] ? jazzman.util.htmlEscape(b[property].toString()) : b[property]) + "' in expected, but was '" + (a[property] ? jazzman.util.htmlEscape(a[property].toString()) : a[property]) + "' in actual.");
        }
    }
    ...
    return function() {
        ...
        if (result) {
            ...
            if (this.message) {
                ...
                message = "Expected " + jazzman.pp(this.actual) + (this.isNot ? " not " : " ") + englishyPredicate;
...
jazzman.Matchers.prototype.toHaveBeenCalledWith = function() {
    ...
    this.message = function() {
        var invertedMessage = "Expected spy " + this.actual.identity + " not to have been called with " + jazzman.pp(expectedArgs) + " but it was.";
        var positiveMessage = "";
        
        if (this.actual.callCount === 0) {
            positiveMessage = "Expected spy " + this.actual.identity + " to have been called with " + jazzman.pp(expectedArgs) + " but it was never called.";
        }
        else {
            positiveMessage = "Expected spy " + this.actual.identity + " to have been called with " + jazzman.pp(expectedArgs) + " but actual calls were " + jazzman.pp(this.actual.argsForCall).replace(/^\[ | \]$/g, '');
        }
...
jazzman.Matchers.prototype.toThrow = function(expected) {
    ...
    if (exception) {
        result = (expected === jazzman.undefined || this.env._contains(exception.message || exception, expected.message || expected));
    }
    ...
    this.message = function() {
        if (exception && (expected === jazzman.undefined || !this.env._contains(exception.message || exception, expected.message || expected))) {
...
jazzman.Matchers.prototype.hasHTML =
    jazzman.Matchers.prototype.toHaveHtml;
...
jazzman.Matchers.ObjectContaining.prototype.jasmineMatches = function(other, mismatchKeys, mismatchValues) {
    ...
    for (property in this.sample) {
        if (!hasKey(other, property) && hasKey(this.sample, property)) {
            ...
            mismatchValues.push("'" + property + "' was '" + (other[property] ? jazzman.util.htmlEscape(other[property].toString()) : other[property]) + "' in expected, but was '" + (this.sample[property] ? jazzman.util.htmlEscape(this.sample[property].toString()) : this.sample[property]) + "' in actual.");
        }
    }
...
jazzman.MultiReporter.prototype.addReporter = function(reporter) {
    ...
    var previousReportFn = this.report;
...
jazzman.PrettyPrinter.prototype.iterateObject = function(obj, fn, scope) {
    ...
    for (property in obj) {
        ...
        fn.call(
            scope,
            property,
            obj.__lookupGetter__ ? (obj.__lookupGetter__(property) !== jazzman.undefined && 
...
jazzman.Suite.prototype.waitsFor = function() {
    ...
    var waitsForFunc = new jazzman.WaitsForBlock(
        this.env,
        _timeout,
        _latchFunction,
        _timeoutMessage,
...
jazzman.Suite.prototype.beforeAll = function(beforeAllFunction) {
    ...
    var block = new jazzman.Block(this.env, beforeAllFunction, this);
...
jazzman.Suite.prototype.afterAll = function(afterAllFunction) {
    ...
    // Ditto
    var block = new jazzman.Block(this.env, afterAllFunction, this);
    ...
    var _next = function() {
        ...
        if (onComplete) {
            ...
            if (isTopSuite) {
                ...
                var fireEmUp = jazzman._setTimeout;
...
jazzman.Spec.prototype.fail = function(e) {
    ...
    var expectationResult = new jazzman.ExpectationResult(params);
...
jazzman.Spec.prototype.execute = function(onComplete) {
    ...
    _next = function() {
        ...
        if (spec.debugBlocks && jazzman.getOptions().debug === true) {
            var blockIdx = jazzman.getOptions().block;
            ...
            if (typeof blockIdx !== 'undefined') {
                ...
                for (i = 0, len = blocks.length; i < len; i++) {
                    ...
                    if (i === blockIdx) {
                        block.func = jazzman.generateDebuggableBlock(block.func);
                    }
                }
            }
            
            jazzman.showDebugPrompt(function() {
...
jazzman.Spec.prototype.spyOn = function(obj, methodName, ignoreMethodDoesntExist) {
    ...
    var spyObj = jazzman.createSpy(methodName);
