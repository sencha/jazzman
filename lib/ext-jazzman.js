    ...
    for (timerId in tasks) {
        ...
        if (!task.$skipTimerCheck && !task.$allowedTimer) {
            ...
            if (jazzman.DEBUGGING_MODE) {
                ...
                jazzman.console.log('Leaked function: ' + fnSource + (task.created ? ',\ncreated at: ' + task.created : ''));
            }
        ...
        if (isGarbage) {
            ...
            if (!el.destroyed) {
                ...
                for (i = 0, len = events.length; i < len; i++) {
                    ...
                    if (listenerCount) {
                        jazzman.console.error(elId + ': event ' + eventName + ' has listeners in ' + specName + '\r\n', event);
...
jazzman.fireMouseEvent = function(target, type, x, y, button, shiftKey, ctrlKey, altKey, relatedTarget) {
    ...
    if (x == null && y == null && Ext.String.endsWith(type, 'move') && Ext.String.endsWith(jazzman.lastMouseEventType, 'down')) {
 ...
 */
jazzman.firePointerEvent = function(target, type, pointerId, x, y, button, shiftKey, ctrlKey, altKey, relatedTarget, pointerType) {
...
jazzman.createTouchList = function(touchList, target) {
    ...
    return jazzman.supportsTouchListConstructor ? new TouchList(touches) : doc.createTouchList ? doc.createTouchList.apply(doc, touches) : touches;
};
...
jazzman.env.addStartupHook(function() {
    ...
    if (Ext.Element) {
        jazzman._bodyRegion = new Ext.util.Region(0, window.innerWidth || document.documentElement.clientWidth, window.innerHeight || document.documentElement.clientHeight, 0);
